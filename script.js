...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (window.gsap && window.ScrollTrigger) {
    ...
    document.querySelectorAll('section, .service-card, .project-card, .timeline-item').forEach(el => {
      el.style.opacity = '1';
...
function openWhatsApp() {
  const url = 'https://wa.me/2348101595840?text=Hi%20Sanni!%20I%20am%20interested%20in%20working%20with%20you.';
...
function initScrollAnimations() {
  ...
  document.querySelectorAll('.service-card, .project-card, .timeline-item, .testimonial').forEach(el => {
...
function initMobileEnhancements() {
  if (window.innerWidth <= 768) {
    ...
    document.addEventListener('touchstart', (e) => {
      ...
      if (element && (element.classList.contains('service-card') || 
